...
# def search_materials_hybrid(params: Dict):
#     """HYBRID search for materials - similar to search_products_hybrid"""
#     import signal
...
#         if params.get("keywords_vector"):
#             base = params["keywords_vector"]
#         elif params.get("material_name"):
#             base = params["material_name"]
#         else:
#             parts = [params.get("material_group", ""), params.get("usage_context", "")]
#             base = " ".join([p for p in parts if p]) or "vật liệu nội thất"
        
#         print(f"\n🔍 Material Query: {base}")
        
#         # 2. Extract main material type
#         # Common material types
#         main_material_types = [
#             "gỗ", "đá", "da", "vải", "kim loại", "nhựa", "kính", "sơn", 
#             "wood", "stone", "leather", "fabric", "metal", "plastic", "glass", "paint"
#         ]
...
#         # Find main material type
#         main_word = None
#         secondary_words = []
        
#         for word in original_words:
#             if word in main_material_types:
#                 main_word = word
#                 break
        
#         # If main word not found, take first word
#         if not main_word and original_words:
#             main_word = original_words[0]
        
#         # Remaining words are secondary
#         secondary_words = [w for w in original_words if w != main_word]
        
#         print(f"🔍 Main word (REQUIRED): '{main_word}' | Secondary: {secondary_words}")
        
#         # 3. Generate vector
#         vector = generate_embedding_qwen(base)
#         if not vector:
#             if hasattr(signal, 'SIGALRM'):
#                 signal.alarm(0)
#             return {"materials": [], "search_method": "failed", "error": "no_vector"}
        
#         # 4. STEP 1: Search with MAIN word in material_name
#         if not main_word:
#             print("⚠️ No main word detected, returning empty")
#             if hasattr(signal, 'SIGALRM'):
#                 signal.alarm(0)
#             return {"materials": [], "search_method": "no_main_word"}
...
#             WHERE description_embedding IS NOT NULL
#                 AND (material_name ILIKE %s OR material_group ILIKE %s)
#             LIMIT 100
#         """
        
#         cur.execute(sql_step1, [f"%{main_word}%", f"%{main_word}%"])
#         candidates = cur.fetchall()
        
#         if not candidates:
#             print(f"ERROR: No materials found with main word '{main_word}'")
#             if hasattr(signal, 'SIGALRM'):
#                 signal.alarm(0)
#             return {
#                 "materials": [],
...
#         # 5. STEP 2: Calculate vector similarity for SECONDARY words
#         SIMILARITY_THRESHOLD = settings.SIMILARITY_THRESHOLD_LOW
#         MIN_SECONDARY_MATCH_RATIO = 0.4  # 40% for materials (more lenient than products)
        
#         # Create vector for SECONDARY query
#         if secondary_words:
#             secondary_query = " ".join(secondary_words)
#             secondary_vector = generate_embedding_qwen(secondary_query)
#         else:
...
#         scored_materials = []
#         for candidate in candidates:
#             material_name = (candidate.get("material_name") or "").lower()
#             material_group = (candidate.get("material_group") or "").lower()
            
#             # Calculate vector similarity
#             similarity = 0.0
#             if candidate.get("description_embedding") and secondary_vector:
#                 candidate_emb = candidate["description_embedding"]
#                 if isinstance(candidate_emb, str):
#                     candidate_emb = json.loads(candidate_emb)
...
#                 1 for word in secondary_words 
#                 if word in material_name or word in material_group
#             )
#             secondary_match_ratio = secondary_match_count / len(secondary_words) if secondary_words else 1.0
...
#                 "id_sap": candidate["id_sap"],
#                 "material_name": candidate["material_name"],
#                 "material_group": candidate.get("material_group"),
#                 "material_subgroup": candidate.get("material_subgroup"),
#                 "similarity": round(similarity, 3),
#                 "secondary_match_count": secondary_match_count,
#                 "secondary_match_ratio": round(secondary_match_ratio, 2),
#                 "final_score": round(final_score, 3)
...
#             # Condition 1: Similarity meets threshold
#             if m["similarity"] < SIMILARITY_THRESHOLD:
#                 continue
            
#             # Condition 2: If secondary words exist, must match enough
#             if secondary_words:
#                 if m["secondary_match_ratio"] >= MIN_SECONDARY_MATCH_RATIO or m["similarity"] >= 0.6:
#                     filtered_materials.append(m)
#             else:
#                 filtered_materials.append(m)
        
#         # Sort by final_score
#         filtered_materials.sort(key=lambda x: x["final_score"], reverse=True)
        
#         # Limit to 15 materials
#         filtered_materials = filtered_materials[:15]
        
#         if filtered_materials:
#             print(f"SUCCESS: Final: {len(filtered_materials)} materials (main: '{main_word}', similarity >= {SIMILARITY_THRESHOLD})")
#             for i, m in enumerate(filtered_materials[:3], 1):
#                 print(f"  {i}. {m['material_name']} (score: {m['final_score']}, sim: {m['similarity']})")
            
#             if hasattr(signal, 'SIGALRM'):
#                 signal.alarm(0)
#             return {
#                 "materials": filtered_materials,
#                 "search_method": "hybrid_material_search",
#                 "main_word": main_word,
#                 "secondary_words": secondary_words
#             }
#         else:
#             print(f"ERROR: No materials meet similarity threshold (>= {SIMILARITY_THRESHOLD})")
#             if hasattr(signal, 'SIGALRM'):
#                 signal.alarm(0)
#             return {
#                 "materials": [],
...
#         }
#     except Exception as e:
#         print(f"ERROR: Material hybrid search error: {e}")
#         import traceback
#         traceback.print_exc()
#         if hasattr(signal, 'SIGALRM'):
#             signal.alarm(0)
#         return {"materials": [], "search_method": "hybrid_failed"}
#     finally:
#         # Always close connection and cancel alarm
#         if conn:
#             try:
#                 conn.close()
#                 print("INFO: Database connection closed in search_materials_hybrid")
#             except Exception as e:
