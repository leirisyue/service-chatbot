...
#                         search_materials_hybrid, search_products_hybrid, 
...
#     """Tìm kiếm NGUYÊN VẬT LIỆU với giá từ material_subprice - HYBRID FIRST"""
#     print(f"params: search_materials + search_materials_hybrid {params}")
    
#     # TIER 1: Try Hybrid first
#     try:
#         result = search_materials_hybrid(params)
#         print(f"INFO: Hybrid material search results - Found {len(result.get('materials', []))} materials")
        
#         # Check if there's a timeout error
#         if result.get("search_method") == "timeout":
#             print("TIMER: Material search timeout - returning empty materials list")
#             return {
#                 "materials": [],
#                 "search_method": "timeout",
#                 "response": "No matching materials found",
#                 "success": False
#             }
...
#             # Enrich with price data from VIEW_MATERIAL_MERGE
#             id_saps = [m['id_sap'] for m in materials]
#             origin_map = _fetch_material_view_data(id_saps)
            
#             materials_with_price = []
#             for mat in materials:
#                 origin_info = origin_map.get(mat['id_sap'], {})
#                 merged_subprice = origin_info.get('material_subprice')
...
#             print(f"SUCCESS: Hybrid search returned {len(materials_with_price)} materials with prices")
#             result["materials"] = materials_with_price
#             result["success"] = True
#             return result
...
#     # TIER 2: Fallback to vector + keyword search
#     print("INFO: Falling back to vector + keyword search")
#     conn = None
#     try:
#         conn = get_db()
#         cur = conn.cursor(cursor_factory=RealDictCursor)
...
#             if results:
#                 # ========== STEP 1.5: QUERY MATCHING BOOST ==========
#                 # Boost base_score if query appears in material fields
#                 query_keywords = query_text.lower().split()
                
#                 for mat in results:
#                     # Set initial base_score from distance
#                     mat['base_score'] = max(0.0, 1.0 - float(mat.get('distance', 0.5)))
                    
#                     boost = 0.0
                    
#                     # Fields to check
#                     material_name = (mat.get('material_name') or '').lower()
#                     material_group = (mat.get('material_group') or '').lower()
#                     material_subgroup = (mat.get('material_subgroup') or '').lower()
#                     id_sap = (mat.get('id_sap') or '').lower()
...
#                         # Boost if keyword appears in material name (most important)
#                         if keyword in material_name:
#                             boost += 0.20
#                             match_count += 1
                        
#                         # Boost if appears in material group
#                         if keyword in material_group:
#                             boost += 0.10
#                             match_count += 1
                        
#                         # Boost if appears in subgroup
#                         if keyword in material_subgroup:
#                             boost += 0.08
#                             match_count += 1
                        
#                         # Boost if appears in SAP ID
#                         if keyword in id_sap:
#                             boost += 0.05
#                             match_count += 1
                    
#                     # Update base_score (max limit 1.0)
#                     if boost > 0:
#                         mat['base_score'] = min(1.0, mat['base_score'] + boost)
#                         mat['query_match_count'] = match_count
#                         mat['query_boost'] = boost
#                         print(f"  INFO: Boosted {mat['id_sap']}: +{boost:.3f} (matches: {match_count})")
#                     else:
#                         mat['query_match_count'] = 0
#                         mat['query_boost'] = 0.0
...
#                             mat_dict['price'] = get_latest_material_price(merged_subprice) if merged_subprice else 0.0
#                             # Keep boost metadata
#                             mat_dict['base_score'] = mat.get('base_score', 0.0)
#                             mat_dict['query_match_count'] = mat.get('query_match_count', 0)
#                             mat_dict['query_boost'] = mat.get('query_boost', 0.0)
#                             materials_with_price.append(mat_dict)
                        
#                         materials_main = [p for p in materials_with_price if p.get('query_boost', 0) >= 1.5]
#                         materials_low_confidence = [p for p in materials_with_price if p.get('base_score', 0) < 0.6]
            
                        
#                         text_response = ""
#                         if len(materials_main)>0:
...
#         # ========== APPLY QUERY MATCHING BOOST TO KEYWORD RESULTS ==========
#         query_keywords = query_text.lower().split()
        
#         for mat in results:
#             # Set initial base_score for keyword search
#             mat['base_score'] = 0.5  # Default for keyword matches
...
#             filtered_results = []
#             for mat in results:
#                 mat_name_upper = mat.get('material_name', '').upper()
...
#             if not results:
#                 return {
#                     "response": f"Không tìm thấy vật liệu chứa '{main_keyword}'.",
#                     "materials": [],
#                     "success": False
#                 }
...
#         print(f"SUCCESS: Keyword search with boost: Found {len(materials_with_price)} materials")
#         return {
#             "materials": materials_with_price,
#             "search_method": "keyword_with_boost",
#             "success": True
#         }
        
#     except psycopg2.OperationalError as e:
#         print(f"ERROR: Database connection error in TIER 2: {e}")
#         return {
#             "materials": [],
#             "search_method": "connection_error",
...
#             try:
#                 conn.close()
#                 print("INFO: Database connection closed in search_materials TIER 2")
#             except Exception as e:
#                 print(f"WARNING: Error closing connection in search_materials: {e}")
